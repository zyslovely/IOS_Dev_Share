...
/**
 * @brief                       设置应用版本号，可以在统计数据的时候区分应用的版本号.
 *
 * @param          channel      应用版本号.
 *
 * @warning                     必需:可选,如果需要修改应用的版本
 *
 *                              调用说明:在需要标识出应用的版本的时候，需要调用此方法
 *
 *                              调用顺序:preInit->setAppVersion.
 *
 *                              最佳实践:[UT setAppVersion:@"3.1.2"]
 *
 *                              最佳位置:didFinishLaunchingWithOptions,preInit之后,init之前
 *
 *
...
// 异常时候 crashhandler 调用，因为crashhandler是独立于内部的一个模块。这里的做法也是想把uninit的实际代码废弃掉。
+ (void) onCrashOccur;
...
+(void) setEventDispatchDelegate : (id<UTEventDispatchProtocol>) delegate;
